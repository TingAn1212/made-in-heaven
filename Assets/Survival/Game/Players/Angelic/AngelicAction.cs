...
public class AngelicAction : MonoBehaviour
{
    GameControl control;
    ...
    void ReadMouse(){
        ...
        if (Input.GetMouseButtonDown(1))
        {
            if (CD["Weapon2"] == 0){
                GameObject.Find("EffectPlayer").GetComponent<EffectControl>().Play(16);
                GameObject NewEntity = Instantiate(Bullet2, GunPoint1.transform.position, Quaternion.identity);
                NewEntity.GetComponent<BulletScript>().SetVelocity(ScaleVector(new Vector2(Front.x,Front.y),(float)0.8));
                control.entitys.Add(NewEntity);
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            if (CD["E"] == 0){
                ...
                GameObject effect = Instantiate(HealEffect, new Vector3(0,0,0), Quaternion.identity);
                control.Particles.Add(effect);
                GetComponent<Data>().HP += 10f;
                CD["E"] = (float)600;
            }
        }
